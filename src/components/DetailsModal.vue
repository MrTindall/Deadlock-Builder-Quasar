<template>
  <q-dialog backdrop-filter="blur(4px) desaturate(-150%)" ref="modal">
    <q-card :class="[item.item_slot_type + '-dark']">
      <q-card-section
        :class="['row items-center', 'text-h6', item.item_slot_type]"
      ...
      <q-card-section
        v-if="item.component_items && item.component_items.length"
      ...
      <div class="q-pa-md q-mb-sm">
        ...
        <div
          v-if="item.properties.BonusAbilityCharges?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.BonusAbilityCharges.value }} Bonus Ability
          Charge(s)
        </div>
        <div
          v-if="item.properties.CooldownReductionOnChargedAbilities?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.CooldownReductionOnChargedAbilities.value }}%
          Cooldown on Charged Abilities
        </div>
        ...
        <div
          v-if="item.properties.AbilityLifestealPercentHero?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.AbilityLifestealPercentHero.value }}% Spirit
          Lifesteal
        </div>
        <div
          v-if="item.properties.BaseAttackDamagePercent?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.BaseAttackDamagePercent.value }}% Weapon Damage
        </div>
        <div
          v-if="item.properties.BonusMeleeDamagePercent?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.BonusClipSizePercent?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.HeadShotBonusDamage?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.HeadShotBonusDamage.value }} Bonus Headshot
          Damage
        </div>
        ...
        <div
          v-if="item.properties.TechArmorDamageReduction?.value"
          class="q-pb-sm"
        >
          {{ item.properties.TechArmorDamageReduction.value }}% Spirit Resist on
          Spirit Damage
        </div>
        <div
          v-if="item.properties.MagicIncreasePerStack?.value > 0"
          class="q-pb-sm"
        >
          +{{ item.properties.MagicIncreasePerStack.value }} Spirit Amp per
          Stack
        </div>
        ...
        <div
          v-if="item.properties.AttackDamageWhenShielded?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.AttackDamageWhenShielded.value }}% Attack Damage
          When Shielded
        </div>
        <div
          v-if="item.properties.BulletShieldMaxHealth?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.BulletShieldMaxHealth.value }} Bullet Shield
        </div>
        <div
          v-if="item.properties.FireRateWhenShielded?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.FireRateWhenShielded.value }}% Fire Rate When
          Shielded
        </div>
        <div
          v-if="item.properties.BonusSpiritWithMagicShield?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.BonusSpiritWithMagicShield.value }} Spirit While
          Shielded
        </div>
        <div
          v-if="item.properties.CooldownReductionWithShield?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.CooldownReductionWithShield.value }}% Cooldown
          Reduction While Shielded
        </div>
        <div
          v-if="item.properties.TechShieldMaxHealth?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.SlowResistancePercent?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.SlowResistancePercent.value }}% Slow Resistance
        </div>
        <div
          v-if="item.properties.TechDamagePercent?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.TechDamagePercent.value }}% Spirit Damage
          (Conditional)
        </div>
        <div
          v-if="item.properties.MovementSpeedSlow?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.TechRadiusMultiplier?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.BonusAbilityDurationPercent?.value"
          class="q-pb-sm"
        >
        ...
        <div
          v-if="item.properties.NonImbuedBonusDuration?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.NonImbuedBonusDuration.value }}% Non-Imbued
          Duration
        </div>

        <div
          v-if="item.properties.ImbuedCooldownReduction?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.ImbuedCooldownReduction.value }}% Imbued Cooldown
          Reduction
        </div>
        <div
          v-if="item.properties.NonImbuedCooldownReduction?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.NonImbuedCooldownReduction.value }}% Non-Imbued
          Cooldown Reduction
        </div>
        ...
        <div
          v-if="item.properties.MovementSpeedBonusDuration?.value > 0"
          class="q-pb-sm"
        >
          {{ item.properties.MovementSpeedBonusDuration.value }}s Move Speed
          Duration
        </div>
        <div
          v-if="item.properties.ProcBonusMagicDamage?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.ProcBonusMagicDamage.value }} Bonus Spirit Damage
        </div>
        <div
          v-if="item.properties.BonusHeavyMeleeDamage?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.BonusHeavyMeleeDamage.value }}% Bonus Melee
          Damage
        </div>
        <div
          v-if="item.properties.MeleeDistanceScale?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.MeleeDistanceScale.value }}% Bonus Melee Distance
        </div>
        <div
          v-if="item.properties.LongRangeBonusWeaponPower?.value > 0"
          class="q-pb-sm"
        >
          + {{ item.properties.LongRangeBonusWeaponPower.value }}% Long Range
          Weapon Damage
        </div>
        <div
          v-if="item.properties.LongRangeBonusWeaponPowerMinRange?.value"
          class="q-pb-sm"
        >
          {{ item.properties.LongRangeBonusWeaponPowerMinRange.value }} Minium
          Distance
        </div>

        <div
          v-if="item.properties.CloseRangeBonusWeaponPower?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.CloseRangeBonusWeaponPower.value }} % Close Range
          Weapon Damage
        </div>
        <div
          v-if="item.properties.CloseRangeBonusDamageRange?.value"
          class="q-pb-sm"
        >
          {{ item.properties.CloseRangeBonusDamageRange.value }} Max Distance
        </div>
        <div
          v-if="item.properties.NonPlayerBonusWeaponPower?.value"
          class="q-pb-sm"
        >
          + {{ item.properties.NonPlayerBonusWeaponPower.value }}% NPC Damage
        </div>
        <div
          v-if="item.properties.NonPlayerBulletResist?.value"
          class="q-pb-sm"
        >
      ...
      <div class="q-pa-md">
        ...
        <div
          v-if="item.properties.BonusClipDuration?.value > 0"
          class="q-pb-sm"
        >
        ...
        <div v-if="item.properties.ActiveBonusMoveSpeed?.value" class="q-pb-sm">
          + {{ item.properties.ActiveBonusMoveSpeed.value }} Move Speed
          (Conditional)
        </div>
      </div>
    ...
    getComponentName(item, arr) {
      if (item.component_items && item.component_items.length > 0) {
        ...
        const foundItem = arr.find(
          (element) => element.class_name === componentName
        );
